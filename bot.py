...
class Instagram():
	...
	def comment_post(self, post, comment):
		...
		try:
			...
			time.sleep(random.randrange(1, 2))
			# time.sleep(100)
			# browser.find_element_by_xpath('/html/body/div[1]/section/main/div/div[1]/article/div[1]/section[1]/span[2]/button').click()
			# browser.find_element_by_xpath('/html/body/div[1]/section/main/div/div[1]/article/div[1]//section[1]/span[2]/button').click()
			browser.find_element_by_xpath('/html/body//span[2]/button').click()
			# browser.find_element_by_xpath('//button[@class="wpO5b  "]').click()
			time.sleep(random.randrange(1, 2))
			# time.sleep(100)
			...
			comment_area = browser.find_element_by_xpath('/html/body//section[3]').find_element_by_tag_name("textarea")
			# comment_area.clear()
			print("send keys")
			...
			browser.find_element_by_xpath('/html/body//section[3]/div/form/button').click()
	...
	def comment_by_tags(self, tags = list(), comments = list(), amount = 10, delay_min = 60, delay_max=600):
		browser = self.browser
		if len(tags) == 0:
...
if config.type_commenting == 'tag':
	bot.comment_by_tags(config.tags, config.comments, config.amount, config.delay_min, config.delay_max)
